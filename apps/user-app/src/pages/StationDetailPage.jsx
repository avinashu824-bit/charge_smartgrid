...
export default function StationDetailPage() {
  ...
  if (!station) return <div className="p-8 text-gray-400">Station not found.</div>;
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div className="bg-gray-800 border border-gray-700 rounded-xl p-2 text-xs">
        ...
        <p className="text-emerald-400 font-bold">₹{payload[0].value}/kWh</p>
      </div>
    );
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-950">
      {/* Header */}
      <div className="sticky top-0 z-10 bg-gray-900/95 backdrop-blur-sm px-4 py-3 border-b border-gray-800 flex items-center gap-3">
        <button onClick={() => navigate(-1)} className="text-gray-400 hover:text-white">
          <ArrowLeft size={22} />
        </button>
        <div className="flex-1 min-w-0">
          <p className="font-semibold truncate">{station.name}</p>
          <p className="text-xs text-gray-400 truncate">{station.zone}</p>
        </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={label} className="bg-gray-900 border border-gray-800 rounded-2xl p-3">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm text-gray-200">{station.address}</p>
            <p className="text-xs text-gray-500 mt-0.5">{station.distanceKm} km from your location</p>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
          ...
          <p className="text-xs text-gray-400 mb-3">₹/kWh by hour</p>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {station.connectorTypes.map(c => (
              <span key={c} className="bg-gray-800 border border-gray-700 rounded-xl px-3 py-1.5 text-xs text-gray-300">
                {c}
              </span>
            ))}
          </div>
        ...
        <button
          ...
          className="w-full bg-emerald-500 hover:bg-emerald-400 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 transition-all shadow-xl shadow-emerald-500/30 text-base"
        >
          <Navigation size={20} />
