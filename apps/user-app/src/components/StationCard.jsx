  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 flex flex-col items-center gap-1">
        ...
        <span className="text-xs text-gray-400">{station.distanceKm}km</span>
      </div>
      ...
      {station.status === 'GREEN' && (
        <div className="flex-shrink-0 text-right">
          <div className="text-xs text-emerald-400 font-bold">Save</div>
          <div className="text-sm font-bold text-emerald-400">₹{station.savingsInr}</div>
        </div>
      )}
    </div>
  );
